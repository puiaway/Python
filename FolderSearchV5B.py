...
DEFAULT_EXTENSIONS = ".txt, .log, .csv, .json, .xml, .md, .py, .ini"
...
class TextSearchApp:
    ...
    def _normalize_exts(self, extensions_str):
        # ".TXT ,log,  .Py" -> ('.txt', '.log', '.py')
        cleaned = []
        for raw in extensions_str.split(','):
            s = raw.strip().lower()
            if not s:
                continue
            if not s.startswith('.'):
    ...
    def search_files(self, folder, keyword, extensions_str):
        ...
        keyword_to_find = keyword if search_params["case_sens"] else keyword.casefold()
        file_exts = self._normalize_exts(extensions_str)
        ...
        if total_files == 0:
            self.ui(self.status_label.config, text="Status: No matching file types found.")
            self.ui(self._set_ui_state, False)
            return

        self.ui(self.progress.configure, maximum=total_files)
        ...
        try:
            ...
            self.ui(messagebox.showerror, "File Error", f"Could not create a temporary file: {e}")
            self.ui(self._set_ui_state, False)
            return
        ...
        if self.stop_flag.is_set():
            self.ui(self.status_label.config, text=f"Status: Search cancelled. Processed {min(self.progress['value'], total_files)} files.")
        else:
            self.ui(self.status_label.config, text=f"Status: Complete. Found {total_matches} match(es) in {total_files} file(s).")

        self.ui(self.show_preview, preview_results, total_matches)
        self.ui(self._set_ui_state, False)
    ...
    def _process_files(self, file_list, writer, keyword, params):
        ...
        for i, file_path in enumerate(file_list, 1):
            ...
            try:
                ...
                with open(safe_path(file_path), 'r', encoding=encoding, errors='ignore') as f:
                    ...
                    for line_num, line in enumerate(f, 1):
                        line_cmp = line if params["case_sens"] else line.casefold()
                        if keyword in line_cmp:
                            found_in_file = True
                            match_data = (filename, line_num, line.rstrip('\n'))
            ...
            # Write results (including optional non-matches)
            if found_in_file or (params["inc_nomatch"] and not matches_in_file):
                ...
                for match in matches_in_file:
                    ...
                    # keep preview light unless "Show all"
                    if params["show_all"] or len(preview) < 1000:
                        preview.append(match)

                if found_in_file:
                    # count only real matches, not the "[No match found]" rows
                    if params["inc_nomatch"]:
                        match_count += sum(1 for m in matches_in_file if m[1] != "-")
                    else:
                        match_count += len(matches_in_file)

            # Progress update (via main thread)
            if i % 10 == 0 or i == total_files:
                self.ui(self.progress.configure, value=i)
                self.ui(self.status_label.config, text=f"Status: Scanning {i}/{total_files} files...")
            ...
            try:
                ...
                for col in ws.columns:
                    ...
                    for cell in col:
                        ...
                        if len(s) > max_len:
                            max_len = len(s)
                    ws.column_dimensions[column].width = min(max_len + 2, 70)
