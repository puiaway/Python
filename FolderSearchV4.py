...
HISTORY_FILE = os.path.expanduser("~/.text_search_keywords.json")
...
class TextSearchApp:
    def __init__(self, master):
        ...
        options_frame.grid(row=2, column=0, columnspan=3, sticky='w', padx=10, pady=2)
        tk.Checkbutton(options_frame, text="Match once per file", variable=self.match_once_var).grid(row=0, column=0, sticky='w', padx=5)
        tk.Checkbutton(options_frame, text="Use last match per file", variable=self.last_match_var).grid(row=0, column=1, sticky='w', padx=5)
        tk.Checkbutton(options_frame, text="Case sensitive", variable=self.case_sensitive_var).grid(row=1, column=0, sticky='w', padx=5)
        tk.Checkbutton(options_frame, text="Show all results", variable=self.show_all_var).grid(row=1, column=1, sticky='w', padx=5)
        tk.Checkbutton(options_frame, text="Include non-matching files", variable=self.include_nomatch_var).grid(row=1, column=2, sticky='w', padx=5)
    ...
    def search_files(self):
        ...
        include_nomatch = self.include_nomatch_var.get()

        if not os.path.isdir(folder) or not keyword:
            messagebox.showerror("Error", "Please select a valid folder and keyword.")
            self.reset_ui()
            return
        ...
        file_list = [os.path.join(root, file)
                     ...
                     for file in files if file.lower().endswith(('.txt', '.log', '.csv', '.json', '.xml'))]
        ...
        for index, file_path in enumerate(file_list, 1):
            ...
            try:
                    ...
                    for i, line in enumerate(f, 1):
                        ...
                        if keyword_clean in line_to_check:
                            ...
                            if match_once:
                                ...
                                if show_all or len(preview_results) < 1000:
                ...
                if not found_match and include_nomatch:
                    writer.writerow([filename, "-", "[No match found]"])
                    if show_all or len(preview_results) < 1000:
                        preview_results.append((filename, "-", "[No match found]"))
        ...
        if match_count == 0:
            ...
            preview_results.append(("No results found", "", ""))
    ...
    def show_preview(self, preview, total_count):
        ...
        if not self.show_all_var.get() and total_count > len(preview):
            ...
            self.result_area.insert(tk.END, "Please use 'Export to CSV' or 'Export to Excel' to see all matches.\n")
